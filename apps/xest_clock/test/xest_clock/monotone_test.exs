defmodule XestClock.Monotone.Test do
  ...
  describe "Monotone immutably" do
  ...
  describe "Monotone statefully" do
    setup %{enum: enum} do
    ...
    test "increasing/1 doesnt consume elements from a stateful source", %{source: source} do
    ...
    test "decreasing/1 doesnt consume elements from a stateful source", %{source: source} do
      assert Stream.repeatedly(source)
             |> Monotone.decreasing()
    ...
    @tag enum: [1, 2, 3, 5, 4, 6]
    test "strict/2 with :asc doesnt consume elements from a stateful source", %{source: source} do
      assert Stream.repeatedly(source)
    ...
    @tag enum: [6, 5, 3, 4, 2, 1]
    test "strict/2 with :desc doesnt consume elements from a stateful source", %{source: source} do
      assert Stream.repeatedly(source)
